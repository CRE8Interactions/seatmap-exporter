...
function applySellOrder(mapping, hotspot, options = {}) {
  if (!mapping?.seats || !hotspot) return mapping;
  ...
  Object.values(nextMapping.seats).forEach((seat) => {
  ...
  if (!distances.length) return nextMapping;
  ...
  Object.values(nextMapping.rows).forEach((row) => {
    const { sortOrder, hotspotFill } = computeRowSortOrder(row, nextMapping);
  ...
  Object.values(nextMapping.sections).forEach((section) => {
    if (section.zoomable) {
      const { sortOrder, hotspotFill } = computeSectionSortOrder(
        section,
        nextMapping
      );
